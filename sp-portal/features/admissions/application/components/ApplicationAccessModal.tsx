...
async function resolveSeededCredentialAccess(schoolId: string, email: string, password: string): Promise<SeededCredentialAccess> {
  ...
  const coreAccessChecks = await Promise.all(
    coreRecords.map(async (record) => {
      ...
      return hasCoordinatorModuleAccessInSupabase(record.id, 'sp_portal');
    }),
  );
  const hasCoreAccess = coreAccessChecks.some(Boolean);
