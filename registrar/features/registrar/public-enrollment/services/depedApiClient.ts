...
export async function fetchDepedSchoolById(id: string): Promise<DepedApiSchoolRecord | null> {
  const normalized = id.trim();
  if (!normalized) return null;
  try {
    const payload = await getJson(`/api/schools/${encodeURIComponent(normalized)}`);
